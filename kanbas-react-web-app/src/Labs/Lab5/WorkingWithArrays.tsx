  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <input value={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <textarea value={todo.description} className="form-control w-50 float-start"
      ...
      <div className="form-check">
        <input type="checkbox" checked={todo.completed} id="completed-check" className="form-check-input ms-1"
              onChange={(e) => setTodo({ ...todo, completed: !todo.completed }) }/>
        <label className="form-check-label" htmlFor="completed-check">completed</label>
      </div>
      <br /><br /><hr />
    </div>
  );
